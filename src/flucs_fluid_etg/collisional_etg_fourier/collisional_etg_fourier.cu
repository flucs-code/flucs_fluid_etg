...
__global__ void find_derivatives(const FLUCS_COMPLEX* fields,
    ...
    const int padded_ikz = padded_indices.padded_ikz;
    ...
    const int ikz = ikz_from_padded_ikz(padded_ikz);

    const int index = index_from_3d<NZ, NX, HALF_NY>(ikz, ikx, padded_iky);
...
__global__ void find_nonlinear_bits(const FLUCS_FLOAT* real_derivatives,
    ...
    // dxphi T
    real_bits[real_index] =\
        real_derivatives[real_index]*real_derivatives[real_index + 2*PADDEDSIZE];

    // dyphi T
    real_bits[real_index + PADDEDSIZE] =\
        real_derivatives[real_index + PADDEDSIZE]*real_derivatives[real_index + 2*PADDEDSIZE];
}
...
__device__ void add_nonlinear_terms(const int index,
    ...
    const int padded_ikz = padded_ikz_from_ikz(ikz);

    const int padded_index = index_from_3d<PADDED_NZ, PADDED_NX, HALF_PADDED_NY>(padded_ikz, padded_ikx, iky);
    ...
    const int multistep_index_0 = ((current_step      % 3 + 3) % 3) * HALFUNPADDEDSIZE + index;
    const int multistep_index_1 = ((current_step + 2) % 3)          * HALFUNPADDEDSIZE + index;
    const int multistep_index_2 = ((current_step + 1) % 3)          * HALFUNPADDEDSIZE + index;
